<script>
	...
	import { grid, userGrid, invalidCells, cellKey } from '@sudoku/stores/grid';
			...
			{#each $userGrid as row, y}
				{#each row as value, x}
					<Cell {value}
					      cellY={y + 1}
					      cellX={x + 1}
					      candidates={$candidates[cellKey(x, y)]}
					      hintCandidates={$hintCandidates[cellKey(x, y)]}
					      disabled={$gamePaused}
					      ...
					      conflictingNumber={$settings.highlightConflicting && $grid[y][x] === 0 && $invalidCells.includes(cellKey(x, y))} />
				{/each}
			{/each}
