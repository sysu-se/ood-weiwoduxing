...
## 附录：HW2 Code Review 反馈修复对照

HW2 的 Code Review（`codex-review.md`）共进行了三轮审查，逐轮修复记录如下。

### 第一轮 Review（7 个缺点）→ 已全部修复

| # | Review 指出的问题 | 修复方式 |
|---|------------------|-------------|
| 1 | `guess()` 自赋值 no-op 操作 | 移除冗余赋值，注释标注可变语义 |
| 2 | `getNextHint()` 端到端路径断裂 | 新增 Actions.svelte "下一步"独立按钮，贯通 Sudoku → Game → Store → UI |
| 3 | `Game.toJSON()` 探索状态序列化不完整 | 完整序列化 6 个探索状态字段，`createGameFromJSON` 通过 `_restoreExploreState()` 恢复 |
| 4 | `handleHint()` 缺 cursor null guard | 新增 `cursor.x === null || cursor.y === null` guard |
| 5 | `abandonExplore()` 对已快照对象二次 `snapshot()` | 移除冗余深拷贝，直接使用 `enterExplore` 保存的 `#exploreOrigin` |
| 6 | `syncFromGame()` 遍历中间 Sudoku 对象 | 新增 `getGridSnapshot()` 直接返回 grid 副本，跳过 Sudoku 构建 |
| 7 | 探索冲突字段命名 `exploreConflict` 与 Game 层不一致 | 四文件重命名为 `exploreWarning`，与 `getExploreWarning()` 对齐 |

### 第二轮 Review（4 个缺点，1 major + 3 minor）

| # | Review 指出的问题 | 严重程度 | 状态 |
|---|------------------|---------|------|
| 1 | "下一步"提示填入后删除数字，提示状态消失 | major | ✓ 已修复（第三轮）。`syncFromGame()` 重算逻辑改为始终保留 hintCandidates 键 |
| 2 | `hints.reset()` 不重置 `hasNextHint` | minor | 未修复 |
| 3 | `syncFromGame()` 职责过重 | minor | 未修复 |
| 4 | Hint 按钮 disabled 表达式空光标风险 | minor | 未修复 |

### 第三轮 Review（11 个缺点，5 major + 6 minor）

第三轮进行了两路并行 Agent 深度审查（领域+Store 层 7 文件，UI 组件层 6 文件）。第二轮 major 缺陷 #1 已修复，但发现以下新问题：

| # | Review 指出的问题 | 严重程度 | 状态 |
|---|------------------|---------|------|
| 1 | `clearAnswers()`/`abandonExplore()` 不清除 `hintCandidates`，提示覆盖层残留 | major | 未修复 |
| 2 | `grid.subscribe` 新游戏回调未重置 `exploreWarning`/`isExploring` | major | 未修复 |
| 3 | Board 坐标体系 0-based 与 1-based 混用 | major | 未修复 |
| 4 | `invalidCells` 与 `Sudoku.isValid()` 重复实现冲突检测 | major | 未修复 |
| 5 | Candidates CSS 与注释矛盾，Hint 与笔记无法视觉区分 | major | 未修复 |
| 6 | `hints.reset()` 不重置 `hasNextHint`（第二轮延续） | minor | 未修复 |
| 7 | `syncFromGame()` 职责过重（第二轮延续） | minor | 未修复 |
| 8 | Hint 按钮 disabled 缺显式空光标保护（第二轮延续） | minor | 未修复 |
| 9 | Keyboard 使用已废弃的 `keyCode` | minor | 未修复 |
| 10 | `handleHint()` 包含过多领域流程在 UI 层 | minor | 未修复 |
| 11 | `gameWon.subscribe()` 无取消订阅 | minor | 未修复 |

核心发现：缺陷 #1（提示覆盖层残留）是第二轮修复（始终保留 hintCandidates 键）的直接连锁反应——键永不删除暴露了提示清理逻辑的缺失。缺陷 #4 揭示了 Store 层存在不委托领域层的独立冲突检测实现，增加规则不一致风险。整体评价：领域层设计扎实，Store 适配层状态同步一致性是最薄弱环节。

### 第四轮 Review（10 个缺点，5 major + 5 minor）

第三轮 11 个缺陷已全部修复。本轮两路并行 Agent 审查发现以下新问题：

| # | Review 指出的问题 | 严重程度 | 状态 |
|---|------------------|---------|------|
| 1 | `grid.subscribe` 新游戏未重置 `gameWon`（writable 回归） | major | 未修复 |
| 2 | `grid.subscribe` 中 `userGrid.set` 在 `gameInstance` 赋值前执行 | major | 未修复 |
| 3 | Keyboard 未过滤修饰键，Ctrl+W/A/S/D 与浏览器快捷键冲突 | major | 未修复 |
| 4 | `candidates.add()` 使用 `delete` 从数组移除元素导致稀疏数组 | major | 未修复 |
| 5 | `Game.guess()` 未将归一化后的值回写到 `move` 对象 | major | 未修复 |
| 6 | WASD/方向键绕过 `$keyboardDisabled` 守卫 | minor | 未修复 |
| 7 | `gameWon` 从 derived 改 writable 丢失响应式自动推导 | minor | 未修复 |
| 8 | `handleNextHint()` 缺少 `gamePaused`/`isExploring` 防御性检查 | minor | 未修复 |
| 9 | `candidates.add()` 方法名与实际 toggle 行为不一致 | minor | 未修复 |
| 10 | Keyboard.svelte 中 `cellKey` 导入但未使用 | minor | 未修复 |

核心发现：缺陷 #1 和 #2 是第三轮将 `gameWon` 从 derived 改为 writable 后引入的回归缺陷（新游戏 state 同步顺序问题）。缺陷 #3 和 #6 是第三轮移除 keyCode 时未补充 modifier 过滤的连锁反应。缺陷 #4 是预存的隐蔽 bug。整体评价：四维度均回升至 good，架构改进显著（`syncFromGame` 拆分、`cellKey` 统一、`invalidCells` 委托、`requestHint` 提取），剩余问题集中在初始化边界和输入过滤上。

```text
┌─────────────────────────────────────────────────────┐
│                    UI 层 (Svelte)                     │
│  Actions.svelte  Cell.svelte  Candidates.svelte      │
│  Board/index.svelte  Keyboard.svelte  ...            │
└──────────────┬──────────────────────────────────────┘
               │ subscribe / dispatch
┌──────────────▼──────────────────────────────────────┐
│            Store 适配层 (Svelte stores)               │
│  grid.js (userGrid)  hints.js  explore.js  game.js   │
│  candidates.js  cursor.js  settings.js               │
│  syncFromGame() 桥接领域层 → 响应式层                  │
└──────────────┬──────────────────────────────────────┘
               │ 委托
┌──────────────▼──────────────────────────────────────┐
│            服务层 (Game + History)                    │
│  Game: 状态机(NORMAL/EXPLORING) 会话控制 提示编排      │
│  History: 多叉树 分支管理 剪枝 Undo/Redo              │
│  提示接口 / 探索接口 / 序列化                          │
└──────────────┬──────────────────────────────────────┘
               │ 委托
┌──────────────▼──────────────────────────────────────┐
│            领域层 (Sudoku)                            │
│  #grid / #question 状态封装                           │
│  getCandidates()  findNextHint()  推理能力            │
│  guess()  isValid()  isSafe()  规则校验              │
│  snapshot()  restoreCell()  不可变支持                │
└─────────────────────────────────────────────────────┘
```
